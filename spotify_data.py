...
# Obter credenciais da API do Spotify
...
def get_top_tracks(year):
    ...
    while len(tracks) < 50:
        for track in results["tracks"]["items"]:
            ...
            tracks.append([year, track_name, artist_name, popularity, release_date, genre_str, track_id])
    ...
    tracks = tracks[:50]
    tracks.sort(key=lambda x: x[3], reverse=True)
...
def save_tracks_to_csv(start_year, end_year, filename="top_tracks.csv"):
    ...
    with open(filename, mode="w", newline="", encoding="utf-8") as file:
        ...
        writer.writerow(["Year", "Track", "Artist", "Popularity", "Release Date", "Genres", "Track ID"])
